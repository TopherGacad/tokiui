...
// PILOT VERSION — parallel copy of components/site/component-preview.tsx.
// Differences vs the live version (everything else is byte-identical):
//   • the hand-rolled `SiteSwitch` (.switch) → tokiui <Switch>
//   • the .badge-accent "● Live" pill        → tokiui <Badge variant="soft" dot>
// The preview tab bar is intentionally LEFT bespoke: tokiui <Tabs> renders one
// panel per value and can't show the "split" (preview + code at once) mode.
                ...
                <div className="preview__form">
                  <div className="preview__form-head">
                    ...
                    <span onMouseEnter={() => setHovered('badge')}>
                      {/* PILOT: .badge-accent → tokiui Badge (soft + dot). Border re-added
                          via inline style to match the bespoke pill's outline. */}
                      <Badge
                        variant="soft"
                        color="default"
                        dot
                  ...
                  <div className="preview__form-row" onMouseEnter={() => setHovered('switch')}>
                    {/* PILOT: hand-rolled .switch → tokiui Switch. Default size (h-5 w-9)
                        matches the bespoke 36×20 exactly; track/border overridden to match.
                        2b FIXED: thumb color now matched via the new `thumbClassName` prop. */}
