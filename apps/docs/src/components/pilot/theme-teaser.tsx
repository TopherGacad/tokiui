...
// PILOT: theme teaser.
//   • preset picker buttons (.theme-picker__btn) → tokiui <Button shape="pill" color="contrast">.
//     2d FIXED: pill shape + neutral foreground-filled "selected" state now come from the library,
//     so only page-specific sizing/typography remains as overrides.
//   • CTA "Open the full playground"             → already tokiui <Button>
//   • The two ThemePreview cards keep their bespoke .btn-demo / .badge-demo ON PURPOSE — they are
...
// Unchanged from the live version — these demo buttons/badges are intentionally bespoke
// (tinted by --t-* fake-preset vars; see file header).
        ...
        {options.map((o) => {
          ...
          return (
            <Button
              ...
              onClick={() => onChange(o)}
              // shape="pill" + color="contrast" (active) now come from the library API.
              // Only page-specific sizing/typography + the exact resting tint remain as overrides.
