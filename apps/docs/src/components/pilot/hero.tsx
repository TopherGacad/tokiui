...
// PILOT: hero. CTAs + the v0.3.0 Badge were already tokiui. Added swaps:
//   • "Now public" announcement pill → <Badge variant="outline"> (it IS pill-shaped, so Badge fits;
//     slightly borderline since it's also a container)
//   • install-command copy button    → <Button size="icon">
//   • install-command bar + prompt    → kept bespoke (specialized widget; no matching component)
//   • $ prompt / command text / bg-grid / title → layout, not components
import { Button, Badge } from '@tokiui/ui'
import { useCopy } from '../site/use-copy'
